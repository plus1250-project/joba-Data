...
#!pip install transformers==4.8.2 -U
#!pip install torch
...
np.random.seed(0)
df = pd.read_csv("./training_data/training_data.csv", header=None)
df.columns = ['news_text','induty_code']
df.sort_values('induty_code')
...
train = df.groupby('induty_code', group_keys=False).apply(sampling, sample_pct=0.8)
test = df.drop(df.index[train.index])
...
#.pt파일로 저장
torch.save(model, "kobert_model.pt")
